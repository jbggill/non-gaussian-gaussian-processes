...
class ODEfuncC(nn.Module):

    def __init__(self, diffeq, conditional=False, divergence_fn="approximate", residual=False, rademacher=False):
        super(ODEfuncC, self).__init__()
        assert divergence_fn in ("brute_force", "approximate")
...
class ODEnetC(nn.Module):
    """
    Helper class to make neural nets for use in continuous normalizing flows
    """

    def __init__(self, hidden_dims, input_shape, context_dim, layer_type="concat", nonlinearity="softplus"):
        super(ODEnetC, self).__init__()
        base_layer = {
            "ignore": cond_layers.IgnoreLinear,
            "squash": cond_layers.SquashLinear,
            "scale": cond_layers.ScaleLinear,
            "concat": cond_layers.ConcatLinear,
            "concat_v2": cond_layers.ConcatLinear_v2,
            "concatsquash": cond_layers.ConcatSquashLinear,
            "concatscale": cond_layers.ConcatScaleLinear,
        }[layer_type]
